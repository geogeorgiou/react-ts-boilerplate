...
//async functions return promises and thats our Return Type
export const createUserEvent = (): ThunkAction<
  ...
  try {
    ...
    //access Redux Recorder state with selectRecorderState func
    // and with getState get current Redux state
    const dateStart = selectDateStart(getState());

    //we don't want to use id for creation!
...
const userEventsReducer = (
  ...
  switch (action.type) {
    ...
    case DELETE_SUCCESS: {
      ...
      //use delete operator to delete id from byIds object
    ...
    case UPDATE_SUCCESS: {
      ...
      //in case TS nags about the same name on other action
      //rename the value
      const {event: updatedEvent} = action.payload;

      //no need to change the allIds here ! not affected
      return {
        ...state,
